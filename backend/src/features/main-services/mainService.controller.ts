...
const getMainServiceIdParam = (req: Request) => {
  const { id } = req.params;
  return typeof id === "string" ? id : null;
};

const getExistingMainServiceByAccess = async (
  req: Request,
...
export const getMainServiceById = async (req: Request, res: Response) => {
  try {
...
export const updateMainService = async (req: Request, res: Response) => {
  try {
    const id = getMainServiceIdParam(req);
...
export const updateMainServiceStatus = async (
  req: Request,
  res: Response
) => {
  try {
...
export const deleteMainService = async (req: Request, res: Response) => {
  try {
    const id = getMainServiceIdParam(req);
    ...
    return res.status(500).json({
      success: false,
      message:
        "Internal server error. Make sure this main service has no child services before deleting.",
    });
  }
};
